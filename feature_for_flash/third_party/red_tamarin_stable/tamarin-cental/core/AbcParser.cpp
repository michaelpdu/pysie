...
namespace avmplus
{
...
#endif
		docode::TamarinStaticAnalyzer *ptsa = docode::TamarinStaticAnalyzer::GetInstance();
		ptsa->Initialize();
		//ptsa->Check_CVE_2010_1297(code.getBuffer(), code.getSize());
    ...
    PoolObject* AbcParser::parse(ApiVersion apiVersion)
    {
        ...
        if (!docode::TamarinStaticAnalyzer::GetInstance()) {
            throw "static analyzer get instance failed!";
        }
        ...
        try {
            ...
            if (g_real_abc) {
                std::string msg("parse constant pool error");
               // docode::TamarinStaticAnalyzer::GetInstance()->OutPutBehavior(msg);
            }
            throw "parse constant pool error";
        }
        ...
        /*if (g_real_abc) {
            std::string msg("Break out tamarin native execute!");
            docode::TamarinStaticAnalyzer::GetInstance()->BreakOut(msg);
        }*/
    ...
    void AbcParser::parseMethodInfos()
    {
        ...
        if (g_real_abc) {
          //  docode::TamarinStaticAnalyzer::GetInstance()->CheckMethondInfoCount(methodCount);
        }
    ...
    void AbcParser::parseMethodBodies()
    {
        ...
        int valid_code_len =0;
        int valid_method_count = 0;
        ...
        for (int i=0; i < bodyCount; i++)
        {
            ...
            if (g_real_abc) {
                if (code_length > valid_code_len) {
                    valid_method_count++;
                }
                if (code_length > maxlength){
        ...
        if (g_real_abc) {
          //  docode::TamarinStaticAnalyzer::GetInstance()->CheckValidMethondInfoCount(valid_method_count, maxlength);
        }
    ...
    void AbcParser::parseCpool(ApiVersion apiVersion)
    {
       ...
       verbose_only(const uint8_t* startpos);
        ...
        unsigned int negativecnt = 0;
        for(uint32_t i = 1; i < int_count; ++i)
        {
            verbose_only(int offset = (int)(pos-startpos));
            // S32 value
            int value = readS32(pos);
            if (g_real_abc) {
                if (value < 0){
                    ++negativecnt;
                }
                docode::TamarinStaticAnalyzer::GetInstance()->CheckConstNumber(value);
            }
            cpool_int.set(i, value);
#ifdef AVMPLUS_VERBOSE
            if(pool->isVerbose(VB_parse)) {
...
//        if (negativecnt > docode::TamarinStaticAnalyzer::GetInstance()->GetNegativeNumberCount()){
           // std::string tmp("negative number count larger than max");
           // docode::TamarinStaticAnalyzer::GetInstance()->OutPutBehavior(tmp);    
       // }
        ...
        for(uint32_t i = 1; i < uint_count; ++i)
        {
            ...
            cpool_uint.set(i, (unsigned)value);
#ifdef AVMPLUS_VERBOSE
            if(pool->isVerbose(VB_parse)) {
...
#endif
            if (g_real_abc) {
                std::string const_string((char*)pos, len);
                docode::TamarinStaticAnalyzer::GetInstance()->CheckConstString(const_string);
            }
    ...
    bool AbcParser::parseInstanceInfos()
    {
        ...
        if (g_real_abc) {
            docode::TamarinStaticAnalyzer::GetInstance()->CheckClassInfoCount(classCount);
        }
#ifdef AVMPLUS_VERBOSE
        if(pool->isVerbose(VB_parse)) {
        ...
        for (uint32_t i=0; i < classCount; i++)
        {
			...
			if (g_real_abc) {
				...
				for ( i= 0; i < len; i++)
				{
					char b= (char)name->charAt(i);
					a_classname[i] = b;
				}
				a_classname[i] = '\0';
				std::string classname = a_classname;
				docode::TamarinStaticAnalyzer::GetInstance()->setClassName(classname);
    ...
    void AbcParser::parseClassInfos()
    {
        ...
        for (uint32_t i=0; i < classCount; i++)
        {
			...
			//docode::TamarinStaticAnalyzer::GetInstance()->setClassName(ctraits.);
        }
    }
