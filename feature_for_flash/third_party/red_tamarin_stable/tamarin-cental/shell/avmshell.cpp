...
#include "../../../FlashML/third_party/tarmarin/TamarinProfile.h"
#ifdef VMCFG_HALFMOON
#include "../halfmoon/hm-main.h"  //  halfmoon::enable_mode
#endif
bool g_real_abc = false;
namespace avmshell
{
#ifdef AVMPLUS_WIN32
    ...
    /* static */
    int Shell::run(char* code, unsigned int code_len)
    {
        TAMARIN_PROFILE_FUNCTION;
        {
            TAMARIN_PROFILE_NAME("GCHEAP_INIT_TIME");
        ...
        try{
            ...
            instance->getParamFromSA(code, code_len);
        ...
        {
            TAMARIN_PROFILE_NAME("GCHEAP_DESTROY_TIME");
            MMgc::GCHeap::Destroy();
    ...
    public:
        ...
        if (NULL != settings.code && 0 != settings.code_len) {
            g_real_abc = true;
            avmplus::ScriptBuffer code = shell->newScriptBuffer(settings.code_len);
            memcpy(code.getBuffer(), settings.code, settings.code_len);
            shell->evaluateScriptBuffer(code, false);
            g_real_abc = false;
        }
...
#define LOGGING(x)
    ...
    void Shell::getParamFromSA(char* code, unsigned int code_len) {
        settings.code = code;
        settings.code_len = code_len;
    }
